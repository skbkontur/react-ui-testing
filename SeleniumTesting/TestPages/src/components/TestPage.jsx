...
var selectItems = [
    'EdiXml',
    'Edifact', 'EancomXml', 'KorusXml', 'CISLinkXml',
];
...
export default class TestPage extends React.Component {
    ...
    render() {
        ...
        return <div>
            ...
            <h4>Link</h4>
            <Link data-tid="link" href="http://www.google.com">google</Link>
            <hr/>
            ...
            <Checkbox 
                ...
                data-tid="checkbox">CheckBox</Checkbox>
            <hr/>
            ...
            <Button 
                data-tid="button" 
                onClick={e => this.setState({ buttonText: buttonText + 'bla' }) }>
                Button
            </Button>
            <Label data-tid="innerLabel">{buttonText}</Label>
            <Button 
                data-tid="disabledButton"
                disabled
                onClick={e => this.setState({ buttonText: buttonText + 'bla' }) }>
                DisabledButton
            </Button>
            ...
            <Select data-tid="select" items={selectItems}
                    value={this.state.type}
                    onChange={e => {this.setState({type: e.target.value})}}
                />
            <Select data-tid="doubleSelect" items={doubleSelectItems}
                    value = {this.state.doubleSelectState}
                    onChange={e => {this.setState({doubleSelectState: e.target.value})}}
                />
            <Label data-tid="doubleSelectLabel">{this.state.doubleSelectState}</Label>
            <hr/>

            <h4>Textarea</h4>
            <Textarea data-tid="textarea" {...this::bind('textAreaValue')}>TextArea</Textarea>
            <hr/>

            <h4>Modal</h4>
            <Button data-tid="openModal" onClick={() => this.setState({showModal: true})}>OpenModal</Button>
            {
                this.state.showModal &&
                <Modal data-tid="modal" onClose={() => this.setState({showModal: false})}>
                    <div><Modal.Header data-tid="modalHeader">Modal</Modal.Header></div>
                    <Modal.Body>
            ...
            <h4>List</h4>
            <div data-tid="list">
                {list.map((x) => this.renderListItem(x))}
            </div>
            <hr />

            <h4>RadioGroup</h4>
            <RadioGroup data-tid="radioGroup" items={radioItems}
            ...
            <hr />
            <h4>Tooltip</h4>
            <Tooltip data-tid="tooltip" render={renderTooltip} trigger='click'>Click me</Tooltip>
            <hr />
            <h4>FxInput</h4>
            <FxInput data-tid="fxInput"
